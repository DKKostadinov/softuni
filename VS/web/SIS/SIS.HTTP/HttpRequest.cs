        ...
        public HttpRequest(string httpRequestAsString)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                if (isInHeader)
                {
                    ...
                    if(headerParts.Length != 2) throw new HttpServerException($"Invalid header: {line}");
            ...
            {
                var parameterPart = bodyPart.Split(new char[] {'='}, 2);
                this.FormData.Add(
                    HttpUtility.UrlDecode(parameterPart[0]), 
                    HttpUtility.UrlDecode(parameterPart[1]));
            }
        }
